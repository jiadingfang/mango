...
def evaluate_model(rst_dir, map_dir, output_dir, task_type):
    ...
    args = [
        (os.path.join(rst_dir, map_name), map_dir, map_name, output_dir, task_type)
        for map_name in map_names
        if map_name in os.listdir(rst_dir)
    ]
    ...
    combined_summary = {"summary": final_summary, **all_summaries}
...
def get_final_summary(list_of_dict):
    ...
    avg_by_valid_keys = [
        k
        for k, v in list_of_dict[0].items()
        if k not in sum_up_key and k not in avg_by_total_keys
    ]
    ...
    for k, v in final_summary.items():
        if k in avg_by_total_keys:
            final_summary[k] = (
                v / final_summary["total_task"]
                if final_summary["total_task"] != 0
                else 0
            )
        if k in avg_by_valid_keys:
            final_summary[k] = (
                v / final_summary["valid_task"]
                if final_summary["valid_task"] != 0
                else 0
            )
