...
def get_game_info_with_G_eval(map_dir: str, game_name: str):
    ...
    G, all2all, all_pairs, actions, locations, walkthrough = get_game_info(
        map_dir, game_name
    )
    ...
    for u, v, data in G.edges(data=True):
        ...
        if action_ in reverse_dict.keys():
            ...
            if G_eval.has_edge(v, u) and reverse_action in set(
                value["action"] for value in G_eval[v][u].values()
            ):
                continue
