...
class MapEvaluator:
    def __init__(self, map_dir: str, map_name: str, key_mapping: Optional[dict] = None):
        (
            ...
            self.all_pairs,
            self.walkthrough,
        ) = get_game_info_with_G_eval(map_dir, map_name)
    ...
    def bfs_get_multi_des(self, src_node, actions, time_limit=60):
        ...
        while queue:
            ...
            if step < len(actions):
                for _, next_node, attr in self.G_eval.out_edges(
    ...
    def eval_actions(self, path, src_node, dst_node, metric):
        actions_gt = list(
            {
                attr.get("action").lower()
                for (_, _, attr) in self.G_eval.edges(data=True)
            }
        )
        path_actions = []
    ...
    def eval_full_path(self, path, src_node, dst_node):
        ...
        if (
            path[0][location_before_key] != src_node
            or path[-1][location_after_key] != dst_node
        ):
            return 0
        for edge in path:
    ...
    def eval_route_finding(self, file_path):
        ...
        for path in self.all2all.values():
            if (
                src_node == path["src_node"]
                and dst_node == path["dst_node"]
                and path["diff_shortest"] == 0
            ):
                return_rst["path_len"] = len(path["path_details"])
                break
        ################################################################################################
        min_step_forward_answerable = self.all_pairs[sample_id][
            "min_step_forward_answerable"
        ]
        ...
        return_rst["loose_score"] = self.eval_actions(
            parsed_response, src_node, dst_node, EvalMetric.Loose
        )
        return_rst["strict_score"] = self.eval_actions(
            parsed_response, src_node, dst_node, EvalMetric.Strict
        )
        return_rst["reasoning_score"] = self.eval_full_path(
            parsed_response, src_node, dst_node
        )
    ...
    def eval_dest_finding(self, file_path):
        ...
        min_step_forward_answerable = self.all2all[sample_id][
            "min_step_forward_answerable"
        ]
        if min_step_forward_answerable > model_cutoff_num:
            return_rst["is_hard"] = 1
        ...
        return_rst["loose_score"] = (
            max(
                [
                    self.matching_score(dst_node, dst_node, EvalMetric.Loose)
        ...
        return_rst["strict_score"] = (
            max(
                [
                    self.matching_score(dst_node, dst_node, EvalMetric.Strict)
                    for dst_node in dst_nodes
                ]
            )
            if len(dst_nodes) > 0
        ...
        return_rst["reasoning_score"] = (
            max(
                self.eval_full_path(parsed_response, src_node, dst_node)
                for dst_node in dst_nodes
            )
            if len(dst_nodes) > 0
            else 0
    ...
    def evaluate_map(self, map_rst_dir, task_type: TaskType):
        ...
        for file in os.listdir(map_rst_dir):
            if file.endswith(".json"):
                ...
                all_info.append({"file": file, "rst": rst})
        ...
        for k in ["all", "easy", "hard"]:
            for key in [
                ...
                if key in [
                    ...
                    result_summary[k][key + "_average"] = (
                        result_summary[k][key] / result_summary[k]["valid_task"]
                        if result_summary[k]["valid_task"] > 0
                        else 0
                    )
                if key in ["parsing_error", "id_error", "step_num_error"]:
                    result_summary[k][key + "_average"] = (
                        result_summary[k][key] / result_summary[k]["total_task"]
                        if result_summary[k]["total_task"] > 0
                        else 0
                    )

        return {"all_info": all_info, "summary": result_summary}
