...
def parse_args():
    ...
    parser.add_argument(
        "--task_type", type=str, choices=["route_finding", "desti_finding"]
    )
    parser.add_argument(
        "--model_name",
        type=str,
...
if __name__ == "__main__":
    ...
    walkthrough_text, cutoff_step_num = model.cutoff_input(
        walkthrough_text, max_token_num, max_step_num
    )

    # action_space
    action_space_path = os.path.join(
        data_folder, game_name, f"{game_name}.actions.json"
    )
    action_space_list = load_json(action_space_path)

    # location_space
    location_space_path = os.path.join(
        data_folder, game_name, f"{game_name}.locations.json"
    )
    ...
    if task_type == "route_finding":
        ...
        all_pairs_path = os.path.join(
            data_folder, game_name, f"{game_name}.all_pairs.jsonl"
        )
        ...
        # all2all
        all2all_path = os.path.join(
            data_folder, game_name, f"{game_name}.all2all.jsonl"
        )
    ...
    batched_sample_list = [
        all_sample_list[i : i + batch_size]
        for i in range(0, len(all_sample_list), batch_size)
    ]
