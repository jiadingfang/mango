...
from mango.inter2data.utils.clean_utils import (
    compute_hash_for_pair,
...
def inter2data(map_path, new_map_path):
    ...
    all2all_file_name = [
        f for f in os.listdir(map_path) if f.endswith(".all2all.json")
    ][0]
    ...
    with open(os.path.join(new_map_path, "all2all.jsonl"), "w") as fw:
        with open(os.path.join(map_path, all2all_file_name), "rb") as fr:
            for line in fr:
                ...
                path["min_step_forward_answerable"] = path[
                ...
                if path["dst_node"] in start_end_dict[path["src_node"]].keys():
                    start_end_dict[path["src_node"]][path["dst_node"]].append(
                        path["min_step_total_answerable"]
                    )
                else:
                    start_end_dict[path["src_node"]][path["dst_node"]] = [
                        path["min_step_total_answerable"]
                    ]
    ...
    all_pairs_file_name = [
        f for f in os.listdir(map_path) if f.endswith(".all_pairs.json")
    ][0]
    ...
    with open(os.path.join(new_map_path, "all_pairs.jsonl"), "w") as fw:
        with open(os.path.join(map_path, all_pairs_file_name), "r") as fr:
            for line in fr:
                ...
                pair["path_min_step_forward_answerable"] = pair[
                    "path_seen_in_forward_answerable"
                ]
                pair["path_min_step_total"] = pair["path_min_steps"]

                if pair["dst_node"] in start_end_dict[pair["src_node"]]:
                    pair["path_min_step_total_answerable"] = start_end_dict[
                        pair["src_node"]
                    ][pair["dst_node"]]
                else:
                    pair["path_min_step_total_answerable"] = []

                pair["min_step_forward"] = (
                    min(pair["path_min_step_forward"])
                    if len(pair["path_min_step_forward"]) > 0
                    else 9999
                )
                pair["min_step_total"] = (
                    min(pair["path_min_step_total"])
                    if len(pair["path_min_step_total"]) > 0
                    else 9999
                )
                pair["min_step_forward_answerable"] = (
                    min(pair["path_min_step_forward_answerable"])
                    if len(pair["path_min_step_forward_answerable"]) > 0
                    else 9999
                )
                pair["min_step_total_answerable"] = (
                    min(pair["path_min_step_total_answerable"])
                    if len(pair["path_min_step_total_answerable"]) > 0
                    else 9999
                )
