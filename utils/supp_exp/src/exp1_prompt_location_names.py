...
def main():
    ...
    for game in sorted(games):
        if game != "temple":
            continue
        walkthrough_path = os.path.join(
            data_folder, game, "{}.walkthrough".format(game)
        )
        ...
        valid_moves_path = os.path.join(
            valid_moves_folder, game, "{}.valid_moves.csv".format(game)
        )
        walkthrough_list = read_valid_moves(valid_moves_path)[: len(descriptions)]
        print("len: {}".format(len(descriptions)))

        golden_examples = [
            format_demo(idx, item, walkthrough_list[idx])
            for idx, item in enumerate(descriptions[:5])
        ]
        ...
        print(result)
        success = [
            walkthrough_list[i] == result[i] for i in range(len(walkthrough_list))
        ]
        success_rate = sum(success) / len(success)
        print(success_rate)
        with open(
            "./{}_extracting_location.txt".format(game), "w+", encoding="utf-8"
        ) as fout:
            fout.write(prompt)
            fout.write("\n")
